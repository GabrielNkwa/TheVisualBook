...
public class ClaimedImagesActivity extends AppCompatActivity {
    ...
    private Button add_photo, viewVideos;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_claimed_images);

        bookCode = getIntent().getExtras().get("gift_key").toString();

        txtVwTAG = (TextView) findViewById(R.id.tag);
        txtVwTAG.setText(bookCode + " Visual Book's Photos");

       // gridView = (GridView) findViewById(R.id.gridView);

        root = FirebaseDatabase.getInstance().getReference("GiftCollection").child(bookCode).child("Images");

        mDBReference = FirebaseDatabase.getInstance().getReference("GiftCollection").child(bookCode).child("Images");
        mSReference = FirebaseStorage.getInstance().getReference();

        add_photo = (Button) findViewById(R.id.addImage_bttn);
        ...
        viewVideos = (Button) findViewById(R.id.vid_button);
        viewVideos.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ClaimedImagesActivity.this, ClaimedVideosActivity.class);
        ...
        switch (touchEvent.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (x1 < x2) {
                 ...
                 //   Intent intent = new Intent(ClaimedImagesActivity.this, ClaimedImagesActivity.class);
                 //   startActivity(intent);
